...
- ⚡ **Preloading**: Next songs download while current plays = instant transitions
...
- [yt-dlp](https://github.com/yt-dlp/yt-dlp) (for preloading songs)
- A Discord account
- A Twitch channel
...
├── cache/              # Preloaded songs (auto-managed)
└── public/
...
A: Make sure mpv and yt-dlp are installed. Test with `mpv --version` and `yt-dlp --version`.

**Q: Songs still take time to start?**
A: The first song streams directly. Songs in queue are preloaded while the current song plays, so they start instantly.
